...
%     .fix_box: have a fixed node spacing on the outer boundaries
%
...
if(mesh_par.fix_box)
   % add regular nodes on side of box
   if(mesh_par.area_glob > 0)
       dx = sqrt(mesh_par.area_glob);
   else
       dx = (x_max-x_min)/5;
   end
   xr = x_min+dx:dx:x_max-dx;
   yr = y_min+dx:dx:y_max-dx;
   SIDES = [];
   x = xr;y=ones(size(xr))*y_min;add = [ x ; y ];SIDES = [ SIDES add];
   x = xr;y=ones(size(xr))*y_max;add = [ x ; y ];SIDES = [ SIDES add];
   y = yr;x=ones(size(yr))*x_min;add = [ x ; y ];SIDES = [ SIDES add];
   y = yr;x=ones(size(yr))*x_max;add = [ x ; y ];SIDES = [ SIDES add];
   no_pts       = size(SIDES,2);

   pts_u        = pts_u + no_pts;
   pts_l        = pts_l+no_pts;
   
   pointlist =    [pointlist SIDES];
...
%        alpha        = [0.5 0.5];
        ...
        dx = 0.01;
        xline = dx:dx:1-dx;
        LINE = [ xline ; ones(size(xline))*0.5 ];
        pointlist    = [pointlist   LINE];
    case 2
        %INCLUSION
        alpha        = [0.75 0.5];
        center_x     = alpha(1)*x_max+(1-alpha(1))*x_min;
